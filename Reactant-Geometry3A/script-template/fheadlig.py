...
#these are the hard coded positons generated by the DFT calculation as well as their respective atom_ids
#if you use a non planner alpha keto acid head you may have to change these positions. 
# Otherwise alignment will not be good or not work at all.


cordlist=[]
headlist=[]
#writes our head and cords to output.txt
...
# print(cordlist)
#actually computes our rotation matrix and translation matrix to get the atoms to align
R,t = kabsch_algorithm(cords,DFTopt)
atoms = extract_all_atoms_manual('LIG.pdb')
#takes are tail and rotates and translates them
newcords = []
for atom in atoms:
...
#writes are new lines to LIG.pdb with translation and rotation for ALL atoms
